...
CREATE TABLE observability_vm_instances (
    ...
    last_node_sequence bigint NOT NULL CHECK (last_node_sequence >= 0),
    last_run_sequence bigint NOT NULL CHECK (last_run_sequence >= 0),
    last_kind text NOT NULL,
    -- The latest run: its start, and its stop once observed. The stop is
    -- the run's first stop: the lowest-sequence stop of the VM on the
    -- run's node past the run's start, whatever order the batches
    -- carrying them arrive in.
    run_node_id uuid,
    run_started_at timestamptz,
    run_start_node_sequence bigint CHECK (run_start_node_sequence >= 0),
    stopped_at timestamptz,
    stop_kind text,
    stop_node_sequence bigint CHECK (stop_node_sequence >= 0),
...
-- A run's stop: the stops only, by VM, node and position, so looking up
-- the first stop past a start reads the stops and nothing else.
CREATE INDEX observability_events_vm_stop_idx
    ON observability_events (((payload->>'vm_id')::uuid), node_id, node_sequence)
    WHERE payload ? 'vm_id' AND starts_with(kind, 'vm_driver.vm_stopped.');
...
-- The last-event and run steps are guarded by the order (at, node_id,
-- node_sequence), so a batch that arrives late never moves a newer value
-- backwards; the stop step is guarded by the run it belongs to, and the
-- outcome step by `at`. The last-event step also takes each VM's row
-- first, so one batch at a time is absorbed per VM. Batches commit in any
-- order (the sink flushes concurrently), so a run's stop may be absorbed
-- before its start: the run step looks the stop up in the events already
-- stored, and the stop step takes an earlier stop over a recorded one.
...
BEGIN
    ...
    -- The latest run: a newer start replaces the run, and its stop with
    -- it — the first stop past the start among the events stored so far,
    -- which include this batch's own rows; none when it has not arrived.
    WITH batch AS (
        SELECT
            ((payload->>'vm_id')::uuid) AS vm_id,
            node_id,
    ...
    SET run_node_id = start.node_id,
        ...
        stopped_at = stop.at,
        stop_kind = stop.kind,
        stop_node_sequence = stop.node_sequence
    FROM (
    ...
    LEFT JOIN LATERAL (
        SELECT event.at, event.kind, event.node_sequence
        FROM observability_events AS event
        -- `payload ? 'vm_id'` keeps every row the next line keeps: it is
        -- the stop index's own predicate, here so that the index applies.
        WHERE event.payload ? 'vm_id'
          AND ((event.payload->>'vm_id')::uuid) = start.vm_id
          AND event.node_id = start.node_id
          AND event.node_sequence > start.node_sequence
          AND starts_with(event.kind, 'vm_driver.vm_stopped.')
        ORDER BY event.node_sequence
        LIMIT 1
    ) AS stop ON true
    ...
    -- The run's stop: the batch's first stop past the run's start on the
    -- run's node, for a run not yet stopped — or stopped by a later stop,
    -- which was borrowed from a later run whose start has not arrived.
    WITH batch AS (
        SELECT
            ((payload->>'vm_id')::uuid) AS vm_id,
            node_id,
    ...
    SET stopped_at = first_stop.at,
        ...
        stop_node_sequence = first_stop.node_sequence
    FROM (
        SELECT DISTINCT ON (stop.vm_id) stop.vm_id, stop.at, stop.kind, stop.node_sequence
        FROM batch AS stop
        JOIN observability_vm_instances AS current ON current.vm_id = stop.vm_id
        WHERE starts_with(stop.kind, 'vm_driver.vm_stopped.')
          AND stop.node_id = current.run_node_id
          AND stop.node_sequence > current.run_start_node_sequence
          AND (current.stop_node_sequence IS NULL
               OR stop.node_sequence < current.stop_node_sequence)
