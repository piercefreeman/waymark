...
mod tests {
    ...
    /// A parameter with no `Default` of its own, so the test fails if
    /// `Noop`'s `Default` ever comes to depend on its parameters'.
    struct NoDefault;
    ...
    fn noop_default_regardless_of_its_parameters() {
        ...
        waymark_vm_driver_hooks::VmStopped::vm_stopped(&noop, &NoDefault);
    }
}
