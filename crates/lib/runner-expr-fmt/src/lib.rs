...
pub fn format_value<T: Format>(expr: &ValueExpr<T>) -> String {
...
fn format_value_inner<T: Format>(expr: &ValueExpr<T>, parent_prec: i32) -> String {
    match expr {
        ValueExpr::Literal(lit) => format_literal(&lit.value),
        ValueExpr::Variable(var) => var.name.clone(),
        ValueExpr::ActionResult(value) => value.for_format(),
