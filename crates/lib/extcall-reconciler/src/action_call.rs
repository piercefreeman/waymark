...
pub struct Handler<WorkerPool> {
    /// Shared worker pool for queuing actions.
    pub worker_pool: Arc<WorkerPool>,
    /// Sends dispatch tokens to the poller for tracking.
    pub tx: mpsc::UnboundedSender<(uuid::Uuid, PromiseStateId)>,
}

impl<WorkerPool> Handler<WorkerPool>
where
    WorkerPool: BaseWorkerPool + Send + Sync,
{
    ...
    ) -> Result<uuid::Uuid, HandleEffectError>
    where
        Converter: waymark_convert_core::Convert<ActionCallArgument, serde_json::Value>,
    {
        ...
        let kwargs: HashMap<String, serde_json::Value> = action_ref
            ...
            .iter()
            .zip(args)
            .map(|(name, arg)| (name.clone(), Converter::convert(arg)))
            .collect();
        ...
        let _ = self.tx.send((dispatch_token, promise_state_id));
...
pub struct Poller<WorkerPool> {
    ...
    pub pending: HashMap<uuid::Uuid, PromiseStateId>,
}

impl<WorkerPool> Poller<WorkerPool>
where
    WorkerPool: BaseWorkerPool + Send + Sync,
{
        ...
        Converter: waymark_convert_core::Convert<serde_json::Value, Exception<Value>>,
    {
        loop {
            ...
            while let Ok((dispatch_token, promise_state_id)) = self.dispatch_rx.try_recv() {
                self.pending.insert(dispatch_token, promise_state_id);
            }
            ...
            tokio::select! {
                Some((dispatch_token, promise_state_id)) = self.dispatch_rx.recv() => {
                    self.pending.insert(dispatch_token, promise_state_id);
                }
                ...
                completions = self.worker_pool.poll_complete() => {
                    ...
                    if let Some(settlements) = process_completions::<Converter, Value>(
                        &self.pending,
                        &self.ack_tx,
                        completions,
                    ) {
...
pub fn new<WorkerPool>(worker_pool: WorkerPool) -> (Handler<WorkerPool>, Poller<WorkerPool>) {
...
fn process_completions<Converter, Value>(
    pending: &HashMap<uuid::Uuid, PromiseStateId>,
    ack_tx: &mpsc::UnboundedSender<uuid::Uuid>,
    completions: impl IntoIterator<Item = waymark_worker_core::ActionCompletion>,
    ...
    Converter: waymark_convert_core::Convert<serde_json::Value, Exception<Value>>,
{
    ...
    for completion in completions {
        if let Some(promise_state_id) = pending.get(&completion.dispatch_token) {
            let resolution = completion_to_resolution::<Converter, Value>(completion.result);
            tracing::debug!(?promise_state_id, "action completed");
