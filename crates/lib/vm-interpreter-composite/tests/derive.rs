...
use waymark_vm_interpreter::{CaptureRuntimeView, ExecutionOutcome, Interpreter};
...
impl<'s> CaptureRuntimeView<'s, TestView> for Sub {
    type Captured = &'s mut TestView;

    fn capture_runtime_view(source: &'s mut TestView) -> Self::Captured {
        source
    }
}
...
fn hooks_chain_in_field_order_while_the_state_holds() {
    ...
    let outcome = composite
        .enter_state(&mut view, TestFrame::initial())
...
fn hook_chain_short_circuits_on_frame_exit() {
    ...
    let outcome = composite
        .before_execute(&mut view, TestFrame::initial())
...
fn hook_effects_are_mapped_into_the_field_variant() {
    ...
    let outcome = composite
        .after_execute(&mut view, TestFrame::initial())
...
fn hook_errors_are_mapped_into_the_field_variant() {
    ...
    let result = composite.enter_state(&mut view, TestFrame::initial());
...
#[test]
fn generated_capture_is_the_identity_over_the_borrow() {
    let mut view = TestView;
    let source: *mut TestView = &mut view;
    let captured = <Composite as CaptureRuntimeView<'_, TestView>>::capture_runtime_view(&mut view);
    assert!(core::ptr::eq(source, &raw mut *captured));
}
