...
//! The range is checked once, on construction, so a backend never sees an
//! instant it cannot hold, and a transport turns the construction error
//! into its own rejection with nothing to know.
//!
...
//! 9999-12-31T23:59:59.999999Z. Resolution is not checked here.
...
#[error(
    "the instant must be within {floor:?}..={ceiling:?}, got {instant:?}",
    floor = FLOOR,
    ceiling = CEILING,
)]
...
impl Timestamp {
    ...
    /// # Panics
    ///
    /// Panics when the clock reads past the year 9999.
    #[cfg(feature = "now")]
    pub fn now() -> Self {
...
const FLOOR: chrono::DateTime<chrono::Utc> = chrono::DateTime::from_timestamp_secs(-62_167_219_200)
    .expect("0000-01-01 is within chrono's range");

/// The latest instant in the range: 9999-12-31T23:59:59.999999Z.
const CEILING: chrono::DateTime<chrono::Utc> =
