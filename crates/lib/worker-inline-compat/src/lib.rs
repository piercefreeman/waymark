...
/// body is called with.
///
/// The bytes are the flavor's arguments payload, whatever the arguments
/// converter reads them as — what no bytes mean is the flavor's too.
///
/// # Panics
///
/// Panics when the bytes do not decode as the flavor's arguments
/// message: the dispatch was encoded moments earlier by the same
/// converter in this very process, so undecodable bytes are a bug, not
/// an outcome the action produced.
