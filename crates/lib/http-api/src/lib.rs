...
/// The API router: the given routes, plus `openapi.json`.
///
...
///
/// Call it on the thread that built `routes`: aide gathers the routes'
/// schemas in a thread-local as each route is built.
...
/// The base document every route contributes to, with the `api_urls` as
/// its servers.
