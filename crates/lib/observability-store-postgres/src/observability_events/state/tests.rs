...
/// Batches commit in any order: a run's stop absorbed before its start
/// still stops the run.
#[tokio::test]
async fn a_stop_absorbed_before_its_start_still_stops_the_run() {
    let store = test_store("observability_store_test_state_stop_before_start").await;
    let node = Node::new();
    let vm_id = waymark_ids::InstanceId::new_uuid_v4();

    // The start takes the lower position, the stop the higher one; the
    // batch carrying the stop lands first.
    let start = node.event(100, vm_id, 0, Observation::VmStarted);
    let stop = node.event(101, vm_id, 1, stopped(StopReason::Cancelled));
    for batch in [[stop], [start]] {
        waymark_observability_events_sink_backend::AppendEvents::append_events(
            &store,
            NESlice::try_from_slice(&batch).expect("non-empty"),
        )
    ...
    let run = instance
        ...
        .as_ref()
        .expect("its start was observed");
    assert_eq!(run.started_at.timestamp(), 100);
    let stop = run.stopped.as_ref().expect("stopped");
    assert_eq!(stop.reason, StopKind::Cancelled);
    assert_eq!(stop.at.timestamp(), 101);
}

/// A later run's stop that lands before that run's start is not left on
/// the earlier run: once the later start arrives, the run is that one and
/// the stop is its own.
#[tokio::test]
async fn a_later_runs_stop_absorbed_before_its_start_follows_its_run() {
    let store = test_store("observability_store_test_state_later_run_stop_first").await;
    let node = Node::new();
    let vm_id = waymark_ids::InstanceId::new_uuid_v4();

    let first_start = node.event(100, vm_id, 0, Observation::VmStarted);
    let second_start = node.event(200, vm_id, 0, Observation::VmStarted);
    let second_stop = node.event(201, vm_id, 1, stopped(StopReason::Cancelled));
    // The first run starts; the second run's stop lands before its start.
    for batch in [[first_start], [second_stop], [second_start]] {
        waymark_observability_events_sink_backend::AppendEvents::append_events(
            &store,
            NESlice::try_from_slice(&batch).expect("non-empty"),
        )
    ...
    assert_eq!(
        ...
        200,
        "the latest run is the second"
    );
    let stop = run.stopped.as_ref().expect("stopped by its own stop");
    assert_eq!(stop.at.timestamp(), 201);
    assert_eq!(instance.last_event.at.timestamp(), 201);
}

/// A run holding a later run's stop gives it up for its own, earlier one:
/// the stop of a run is the first stop past its start on its node, however
/// the batches land.
#[tokio::test]
async fn a_runs_own_earlier_stop_replaces_a_later_runs_stop() {
    let store = test_store("observability_store_test_state_earlier_stop_replaces").await;
    let node = Node::new();
    let vm_id = waymark_ids::InstanceId::new_uuid_v4();

    let first_start = node.event(100, vm_id, 0, Observation::VmStarted);
    let first_stop = node.event(101, vm_id, 1, stopped(StopReason::Cancelled));
    let second_start = node.event(200, vm_id, 0, Observation::VmStarted);
    let second_stop = node.event(201, vm_id, 1, stopped(StopReason::Cancelled));
    let append = async |event: Event| {
        waymark_observability_events_sink_backend::AppendEvents::append_events(
            &store,
            NESlice::try_from_slice(&[event]).expect("non-empty"),
        )
        .await
        .expect("append events");
    };
    let stopped_at = async || {
        let instance = store
            .get_instance(vm_id)
            .await
            .expect("get instance")
            .expect("known");
        let run = instance.latest_run.expect("its start was observed");
        (
            run.started_at.timestamp(),
            run.stopped.map(|stop| stop.at.timestamp()),
        )
    };

    // The first run starts, and the second run's stop lands before the
    // first run's own: the first run takes it for now.
    append(first_start).await;
    append(second_stop).await;
    assert_eq!(stopped_at().await, (100, Some(201)));

    // The first run's own stop is earlier past its start, so it replaces
    // the second run's.
    append(first_stop).await;
    assert_eq!(stopped_at().await, (100, Some(101)));

    // The second run starts and takes its own stop.
    append(second_start).await;
    assert_eq!(stopped_at().await, (200, Some(201)));
}
...
async fn instance_list_uses_its_index() {
    ...
    // the choice the query shape allows, not the size heuristic. One
    // connection for the setting and the EXPLAIN: it is per session.
