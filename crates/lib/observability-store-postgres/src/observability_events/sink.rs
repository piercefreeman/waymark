...
fn push_event<'args, Payload>(
    ...
    Payload: waymark_observability_events_core::EventKind + serde::Serialize,
    Payload::Kind: Into<&'static str>,
{
    let kind: &'static str =
        waymark_observability_events_core::EventKind::kind(&event.payload).into();
