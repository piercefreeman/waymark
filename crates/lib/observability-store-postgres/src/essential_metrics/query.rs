...
/// corruption — surfaced wherever a stored histogram column is decoded
/// as it is. The series read decodes its own sums instead, one per
/// bucket of the metric, so it never sees a stored row's width.
///
/// Surfaces as [`sqlx::Error::Decode`], downcastable to this type.
#[derive(Debug, thiserror::Error)]
...
/// The latest read's statement.
///
/// The newest rows are found by walking the primary key downwards: the
/// first step takes the very last row of the key, which is the highest
/// node id's newest sample, and every step after that jumps to the newest
/// sample of the next node id below, one index descent each, carrying the
/// whole row. The table is never scanned, however many samples the
/// retention window holds, and no other index can serve a range on the
