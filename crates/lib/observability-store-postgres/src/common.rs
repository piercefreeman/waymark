...
pub(crate) async fn delete_before_in_chunks(
    ...
    // statement's cached generic plan reads the table from the start for
    // every chunk (a LIMIT makes that look cheap to the planner), and the
    // chunk that ends the sweep — the one that comes up short — reads all
    // of it. Ordered, the index serves every chunk.
    //
