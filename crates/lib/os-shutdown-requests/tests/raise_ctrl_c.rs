//! Raises SIGINT at the test process itself and checks the latching
...
async fn expect_request(receiver: &mut waymark_os_shutdown_requests::ctrl_c::Receiver) {
    tokio::time::timeout(std::time::Duration::from_secs(5), receiver.recv())
        .await
        .expect("request not delivered in time");
}

async fn expect_quiet(receiver: &mut waymark_os_shutdown_requests::ctrl_c::Receiver) {
    let pending =
...
async fn requests_between_waits_are_latched() {
    let mut receiver = waymark_os_shutdown_requests::ctrl_c::install().unwrap();
    ...
    let mut second = waymark_os_shutdown_requests::ctrl_c::install().unwrap();
    raise();
    expect_request(&mut receiver).await;
