    ...
    ActionCallRequester::Argument: serde::Serialize + Send + Sync,
{
    ...
    async fn request_action(
        ...
        // The conservative base for the lock's local fence deadline: it
        // predates the flush instant the stored lock expiry is computed
        ...
        if let RecordOutcome::AlreadyRecorded = outcome {
            tracing::debug!(
                ?promise_state_id,
