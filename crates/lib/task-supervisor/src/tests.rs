...
#[tokio::test]
async fn concurrent_early_ends_are_all_attributed() {
    let shutdown_token = tokio_util::sync::CancellationToken::new();
    let mut supervisor = start::<TaskError>(shutdown_token.clone());

    // Both end on their first poll, before the supervising task sees either
    // end, so both sample the token untouched.
    supervisor.spawn("breaker-1", break_now());
    supervisor.spawn("breaker-2", break_now());

    let report = supervisor.drain().await;
    assert_eq!(report.ended.len(), 2);
    assert!(
        report.ended.iter().all(|ended| ended.before_shutdown),
        "{report}"
    );
}

#[tokio::test]
async fn a_panic_still_shuts_the_others_down() {
    let shutdown_token = tokio_util::sync::CancellationToken::new();
    let mut supervisor = start::<TaskError>(shutdown_token.clone());

    supervisor.spawn("loop", until_shutdown(shutdown_token.clone()));
    supervisor.spawn("panicker", panic_now());
    ...
    let panicker = report
        .ended
        .iter()
        .find(|ended| ended.name == "panicker")
        .unwrap();
    assert!(panicker.before_shutdown);
    assert!(
        matches!(panicker.result, Err(report::Cause::Join(ref join_error)) if join_error.is_panic()),
        "{:?}",
        panicker.result
    );
...
#[tokio::test]
async fn a_panic_and_a_break_together_are_both_attributed() {
    let shutdown_token = tokio_util::sync::CancellationToken::new();
    let mut supervisor = start::<TaskError>(shutdown_token.clone());

    supervisor.spawn("panicker", panic_now());
    supervisor.spawn("breaker", break_now());
