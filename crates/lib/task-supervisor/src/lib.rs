...
//!   task that [`start`] runs, so it does not depend on the owner polling
...
/// Supervise one task: await it to its end and record the end. The
/// task's cancel guard comes back with the end, for the supervising task
/// to drop once every end already in has been recorded: an early end then
/// requests the shutdown only after the ends that came with it were
/// attributed.
async fn supervise<TaskError>(
    supervised_task: SupervisedTask<TaskError>,
) -> (report::End<TaskError>, tokio_util::sync::DropGuard)
where
    ...
    TaskError: std::fmt::Display,
{
    ...
    loop {
        tokio::select! {
            ...
            Some(first) = ends.next(), if !ends.is_empty() => {
                // Every end already in is attributed before any of their
                // guards requests the shutdown, so ends that came together
                // are attributed alike. A task ending from here to the
                // drop below is attributed after the shutdown.
                let mut batch = vec![first];
                while let Some(Some(next)) = ends.next().now_or_never() {
                    batch.push(next);
                }
                for (end, cancel_on_end) in batch {
                    ended.push(end);
                    drop(cancel_on_end);
                }
            }
