...
pub struct End<TaskError> {
    ...
    /// Whether the token was not yet cancelled when the supervisor
    /// observed the end.
    ///
    /// `true` is the early exit: the supervisor cancelled the token in
    /// response, once every end already in was observed, so ends that
    /// came together are attributed alike. `false` is an end after the
    /// shutdown was requested, or one that came in the instant between
    /// that observation and the cancel.
    ...
    TaskError: std::fmt::Display,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(
            f,
            "{} task(s) ended, {} of them before shutdown was requested",
            self.ended.len(),
            early
        )?;
