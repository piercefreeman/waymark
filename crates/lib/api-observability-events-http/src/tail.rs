...
fn docs(op: aide::transform::TransformOperation) -> aide::transform::TransformOperation {
    op.summary("One node's stream in its own order, one page at a time.")
        .description(
            "Ascending by position in the node's stream. A jump in position is a dropped or a \
             late event. When nothing more has landed, the page is empty; ask again later with \
             the last cursor.",
        )
