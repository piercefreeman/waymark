...
pub struct Spawned {
    ...
    /// Notified when the driver thread exits (the VM is evicted).
    driver_evicted: Arc<Notify>,
...
impl Spawned {
    ...
    /// (the VM is evicted).
    pub fn evicted(&self) -> tokio::sync::futures::Notified<'_> {
        self.driver_evicted.notified()
    }
    ...
    <Effector as waymark_vm_driver_core::PromiseSettler>::Error: Send,
{
    ...
    let driver_evicted = Arc::new(Notify::new());
    ...
    let evicted = Arc::clone(&driver_evicted);
