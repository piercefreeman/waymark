...
/// which is created if missing: every connection defaults its
/// `search_path` to it and carries the config's statement timeout, so
/// the consumer's queries stay unqualified.
///
