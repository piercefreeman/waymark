...
fn deserializes_whole_seconds() {
    ...
    assert_eq!(
        seconds.0,
        waymark_nonzero_duration::NonZeroDuration::from_secs(60).expect("non-zero")
    );
}
...
fn schema_is_an_inline_integer_from_one() {
    /// Something holding a duration.
    #[derive(schemars::JsonSchema)]
    struct Holder {
        /// The bucket's width.
        _bucket: NonZeroSeconds,
    }

    let schema = schema_of::<Holder>();

    assert_eq!(
        schema["properties"]["_bucket"],
        serde_json::json!({
            "type": "integer",
            "format": "uint64",
            "minimum": 1,
            "description": "The bucket's width.",
        }),
        "{schema}"
    );
    assert!(schema.get("components").is_none(), "{schema}");
}
