...
// The wire form is whole seconds, so a duration with a fraction of a second
// loses the fraction here; on the wire only whole seconds ever arrive.
impl serde::Serialize for NonZeroSeconds {
    ...
    {
        serializer.serialize_u64(self.0.get().as_secs())
    }
}
