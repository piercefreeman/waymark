...
/// An id the API names: its schema is a component called [`NAME`], so
/// every field holding the id refers to the one definition.
///
/// Implemented for the ids the API serves, and only those.
///
/// [`NAME`]: WellKnownId::NAME
pub trait WellKnownId {
