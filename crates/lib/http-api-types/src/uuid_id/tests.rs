...
fn rejects_what_is_not_a_uuid() {
    ...
    assert!(error.to_string().contains("UUID parsing failed"), "{error}");
}
...
fn schema_is_a_component_named_after_the_id() {
    ...
    // The draft-07 transform (schemars' `RemoveRefSiblings`) allows no
    // siblings on a reference, so the field's own description rides beside
    // the reference wrapped in `allOf`.
    assert_eq!(
...
#[test]
fn every_id_is_well_known_under_its_own_name() {
    assert_eq!(<waymark_ids::NodeId as crate::TypeName>::NAME, "NodeId");
    assert_eq!(
        <waymark_ids::InstanceId as crate::TypeName>::NAME,
        "InstanceId"
    );
    assert_eq!(
        <waymark_ids::ExecutionId as crate::TypeName>::NAME,
        "ExecutionId"
    );
    assert_eq!(
        <waymark_ids::WorkflowVersionId as crate::TypeName>::NAME,
        "WorkflowVersionId"
    );
}
