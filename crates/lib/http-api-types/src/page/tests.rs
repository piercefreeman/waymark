...
#[test]
fn schema_documents_the_cursor_as_a_string_or_null() {
    ...
    assert!(next.get("nullable").is_none(), "{next}");
    assert_eq!(
        next["description"],
        "Where to resume from for what follows this page, as an opaque\ncursor for the read's `after`; null when the page is empty.",
        "{next}"
    );
    assert!(
        schema["required"]
            .as_array()
            .is_some_and(|required| required.iter().any(|name| name == "next")),
        "{schema}"
    );
}
