...
impl<const N: usize> BucketedHistogram<N> {
    ...
    pub fn quantile<const M: usize>(&self, bounds: &[f64; M], quantile: Quantile) -> Option<f64> {
        // One count per bound, then the total: bounds of another length
        // do not build.
        const { assert!(N == M + 1) };
...
/// observations at or below the quantile's value.
///
/// Zero is excluded because it covers no observation, and nothing above one
