...
///
/// The loop runs only once awaited or spawned by the caller, on a tokio
/// runtime: it runs each action it serves as its own task, and ends once
/// every [`Pool`] handle is dropped and every action it took has completed.
/// If the loop is dropped instead, the actions in flight are aborted with
/// it.
pub fn run(
    actions: HashMap<String, InlineActionCallable>,
) -> (
    ...
    loop {
        tokio::select! {
            ...
            Some(joined) = in_flight.join_next() => {
                // A body that panicked takes the loop down with it: the
                // panic is re-raised as is, for whoever runs the loop to
                // report. Nothing here cancels a task, so a join error
                // is only ever a panic.
...
async fn serve(
    ...
    // to an outcome right here, so there is no worker to lose. The
    // completion queue may already be closed by then, and the report has
    // nowhere to go.
    let _ = completion_tx
