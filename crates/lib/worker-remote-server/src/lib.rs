...
use waymark_worker_remote_core::WorkerChannels;

/// gRPC service implementation for the WorkerBridge.
#[derive(Clone)]
pub struct WorkerBridgeService {
    state: Arc<waymark_worker_remote_pending_connections::PendingConnections>,
}

impl WorkerBridgeService {
    fn new(state: Arc<worker-remote-connecting>) -> Self {
        Self { state }
    }
}
...
pub struct WorkerBridgeServer {
    addr: SocketAddr,
    state: Arc<Mutex<waymark_worker_remote_connecting::Registry>>,
...
impl WorkerBridgeServer {
    ...
    pub async fn start(bind_addr: Option<SocketAddr>) -> AnyResult<Arc<Self>> {
        ...
        let state = Arc::new(worker-remote-connecting::new());
    ...
    pub async fn reserve_worker(&self) -> (u64, oneshot::Receiver<WorkerChannels>) {
