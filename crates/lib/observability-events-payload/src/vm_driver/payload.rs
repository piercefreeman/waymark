...
#[schemars(rename = "VmDriverPayload")]
pub struct Payload {
...
pub enum Observation {
    ...
    /// A promise settlement has reached the VM driver.
    PromiseSettled {
        /// The settled promise.
        #[schemars(with = "usize")]
...
pub enum EffectSummary {
    ...
    ActionCall {
        ...
        /// The module the action lives in, when the reference carries one.
...
/// Why a VM driver run stopped: the VM driver's own error, one variant
/// each, with the collaborator's error rendered where the VM driver carries
/// one.
#[derive(Debug, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[serde(tag = "kind", rename_all = "snake_case")]
pub enum StopReason {
