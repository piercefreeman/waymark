...
//! scalar binary and unary operations, simple assignments, conditionals,
//! `try`/`except`, `while` and `for` loops, `break`/`continue`, returns,
//! user function calls, built-in `isexception(...)` checks, and action calls.
//!
