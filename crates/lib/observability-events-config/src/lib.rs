...
pub enum FromEnvError {
    ...
    #[error("int or default: {0}")]
    IntOrDefault(#[source] envfury::Error<envfury::OrParseError<std::num::ParseIntError>>),

    /// A boolean variable could not be read.
    #[error("bool or default: {0}")]
    BoolOrDefault(#[source] envfury::Error<envfury::OrParseError<std::str::ParseBoolError>>),

    /// The lossy batcher policy asks for more concurrent flushes than its
    /// buffers can hold.
    #[error("lossy batcher policy: {0}")]
    LossyBatcherPolicy(#[source] waymark_lossy_batcher::ValidateError),
}
