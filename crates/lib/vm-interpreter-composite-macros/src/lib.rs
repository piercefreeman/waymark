...
/// - the identity `CaptureRuntimeView` implementation for the composite's
///   own runtime view.
///
/// # Attributes
///
...
///     // the composite's runtime view becomes `&'r mut` of this type
...
fn expand(input: syn::DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let capture_impl = expand_identity_capture_impl(&input, &config)?;
...
fn expand_interpreter_impl(
        ...
        quote! {
            ...
            > {
                #(
                    ...
                    let captured =
                        <#field_types as #vm_interpreter_path::CaptureRuntimeView<
                            '_,
                            _,
    ...
    Ok(quote! {
        ...
        #where_clause
        {
            type RuntimeView<'r> = &'r mut #view_ty;
            type Frame = #frame_ty;
...
/// Generate the identity `CaptureRuntimeView` implementation for
/// the composite's own runtime view.
fn expand_identity_capture_impl(
    input: &syn::DeriveInput,
    config: &CompositeConfig,
) -> syn::Result<proc_macro2::TokenStream> {
    let struct_ident = &input.ident;
    ...
    // The struct's generic parameters with defaults stripped, since defaults
    // are not permitted on `impl` generics.
    ...
    Ok(quote! {
        impl<'r, #(#generic_params),*> #vm_interpreter_path::CaptureRuntimeView<'r, #view_ty>
            for #struct_ident #ty_generics
        #where_clause
        {
            type Captured = &'r mut #view_ty;

            fn capture_runtime_view(source: &'r mut #view_ty) -> Self::Captured {
                source
            }
        }
    })
