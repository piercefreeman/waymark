...
//!
//! A read states in its parameters how large a page it is willing to
//! serve, as the type: `Limit<1000>` is a page of one to a thousand. The
//! cap is checked once, on construction, so a backend never sees a page
//! size the read did not agree to, and a transport turns the
//! construction error into its own rejection without checking the cap
//! again.
