...
/// An absolute value below the count never lowers it: the counter is at
/// least every absolute value it was given.
#[test]
fn counter_absolute_never_lowers_the_count() {
    let cell = counter::Cell::default();

    metrics::CounterFn::absolute(&cell, 10);
