...
/// extra one is the bucket above the last bound, which no bound caps.
///
...
impl<const N: usize> Cell<N> {
    ...
    ///
    /// Each bucket's count is taken with one swap to zero, and an
    /// observation adds to its bucket with one increment, so the
    /// observation's count lands either in the interval closing or in the
    /// one opening — never in both, and never in neither. The sum
    /// is read on its own after them, so an observation's count and its
    /// share of the sum can land on different sides of the drain.
