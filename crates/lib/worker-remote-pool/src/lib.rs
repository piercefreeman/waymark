...
/// Publish the dispatch queue's occupancy: the action requests holding a
/// slot in the queue between `queue` and the worker pool loop that hands
/// them to workers. Read the same way on both sides of the queue — after
/// a request is queued and after the loop takes one — so the series means
/// one thing whichever side wrote last.
fn record_dispatch_queue_slots_in_use(max_capacity: usize, capacity: usize) {
    metrics::gauge!("waymark_worker_remote_pool_dispatch_queue_length")
        .set(max_capacity.saturating_sub(capacity) as f64);
}

/// Publish the action requests waiting in the dispatch queue as the worker
/// pool loop sees them after taking the next one. Only the loop can count
/// the waiting requests themselves, so this side alone writes it.
