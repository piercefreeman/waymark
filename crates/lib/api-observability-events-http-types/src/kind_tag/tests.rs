...
fn schema_is_a_component_named_by_the_marker_listing_every_tag() {
    ...
    /// Something holding a kind.
    #[derive(schemars::JsonSchema)]
    struct Holder {
        /// The event's kind.
        _kind: KindTag<TestKind, Name>,
    }

    let schema = schema_of::<Holder>();

    assert_eq!(
        schema["components"]["schemas"]["TestKind"],
        serde_json::json!({
            "type": "string",
            "enum": ["test.started", "test.noted", "test.stopped"],
        }),
        "{schema}"
    );
    assert_eq!(
        schema["properties"]["_kind"],
        serde_json::json!({
            "allOf": [{ "$ref": "#/components/schemas/TestKind" }],
            "description": "The event's kind.",
        }),
        "{schema}"
    );
}
