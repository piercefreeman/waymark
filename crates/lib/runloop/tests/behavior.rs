...
"#;
    ...
    assert_eq!(map.get("y"), Some(&serde_json::Value::Number(8.into())));
}
...
"#;
    ...
    actions.insert(
        ...
        Arc::new(|_kwargs| {
            Box::pin(async move {
                ...
                Ok(serde_json::Value::String("late".to_string()))
            })
        }),
    );
    let worker_pool = waymark_worker_inline::InlineWorkerPool::new(actions);
    ...
    let serde_json::Value::Object(error_obj) = instances_done[0]
        .error
        .clone()
    ...
    assert_eq!(
        error_obj.get("type"),
        Some(&serde_json::Value::String("ActionTimeout".to_string()))
    );
}
...
"#;
    ...
    let serde_json::Value::Object(error_obj) = error else {
        panic!("expected error payload object");
    };
    assert_eq!(
        error_obj.get("type"),
        Some(&serde_json::Value::String("ExecutionError".to_string()))
    );
...
"#;
    ...
    actions.insert(
        ...
        Arc::new(|kwargs| {
            Box::pin(async move {
                ...
                let final_value = kwargs
                    .get("final")
                    .cloned()
                    .unwrap_or(serde_json::Value::Null);
                let iterations = kwargs
                    .get("iterations")
                    .cloned()
                    .unwrap_or(serde_json::Value::Null);
    ...
    let serde_json::Value::Object(result_map) = map
        .get("result")
        .cloned()
    ...
    assert_eq!(
        result_map.get("limit"),
        Some(&serde_json::Value::Number(4.into()))
    );
    assert_eq!(
        result_map.get("final"),
        Some(&serde_json::Value::Number(4.into()))
    );
    assert_eq!(
        result_map.get("iterations"),
        Some(&serde_json::Value::Number(4.into()))
    );
}
...
"#;
    ...
    let mut bootstrap_executor =
        waymark_runner::RunnerExecutor::new(Arc::clone(&dag), state, HashMap::new(), None);
...
"#;
    ...
    actions.insert(
        ...
        Arc::new(|_kwargs| {
            Box::pin(async move {
                Ok(serde_json::Value::Object(
                    [("present".to_string(), serde_json::Value::Number(1.into()))]
                        .into_iter()
                        .collect(),
                ))
            })
...
"#;
    ...
    actions.insert(
        ...
        Arc::new(|_kwargs| Box::pin(async move { Ok(serde_json::Value::Number(7.into())) })),
    );
    let worker_pool = waymark_worker_inline::InlineWorkerPool::new(actions);
    let mut runloop = RunLoop::new(
        worker_pool,
