//! Wire types shared by the nodes operations.
...
pub struct Histogram {
    ...
    /// The interpolated median; null when nothing was observed, or when
    /// the median falls above the last bound.
    pub p50: Option<f64>,
}

fn histogram<const N: usize, const M: usize>(
    histogram: waymark_essential_metrics_core::BucketedHistogram<N>,
    bounds: &[f64; M],
) -> Histogram {
    // A histogram has one bucket past its last bound: bounds of another
    // length do not build.
    const { assert!(N == M + 1) };
    ...
    Histogram {
        ...
        sum: histogram.sum,
        p50: histogram.quantile(bounds, waymark_essential_metrics_core::Quantile::MEDIAN),
    }
}
...
pub struct NodeSample {
    /// The sampling node's id: one identity per node boot.
    ...
    /// Action dispatches accepted into the worker pool's dispatch queue
    /// and not yet taken by its launch loop.
    ...
    /// When the node last completed an action; null before the first
    /// completion.
