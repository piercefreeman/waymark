...
/// `shutdown_token` — and the observability-events API router over the
/// same backend, and the node's emitter for producers to share.
///
...
where
    ...
    <Backend as waymark_observability_events_query_backend::HasPayload>::Payload:
        waymark_observability_events_core::Kinded + serde::Serialize,
{
