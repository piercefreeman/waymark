...
/// The kind some text stands for, if any.
///
/// The way back from text that came from outside — a filter, a stored
/// column — which may stand for no kind at all.
pub trait FromTag: Sized {
    /// The kind `tag` stands for.
...
pub trait SubsetExt: Subset {
    ...
    /// The subset's kind a root kind's tag stands for, if the subset has it.
