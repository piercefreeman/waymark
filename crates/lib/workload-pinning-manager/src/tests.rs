...
fn outcome_display_lists_the_failed_loops() {
    ...
    insta::assert_snapshot!(outcome.to_string(), @"workload pinning manager: poll loop: poll: connection reset; maintenance loop: maintenance loop force shutdown; unpin loop: gave up on pending unpins");
}

#[test]
fn outcome_display_of_a_clean_run_names_no_loop() {
    let outcome = crate::RunOutcome::<&str, &str, &str> {
        poll_error: None,
    ...
    insta::assert_snapshot!(outcome.to_string(), @"workload pinning manager");
}
