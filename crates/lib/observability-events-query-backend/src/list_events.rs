...
pub struct Params<Cursor> {
    ...
    /// newest.
    ///
    /// A cursor is only meaningful with the `from` and `to` it was read
    /// with: keep them fixed for the whole walk.
...
/// a page already read, is never returned by later pages. To follow a
/// node's stream, use [`Tail`](crate::Tail).
