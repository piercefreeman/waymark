...
impl proto::workflow_service_server::WorkflowService for BridgeService {
    ...
    async fn execute_workflow(
        ...
        let skip_sleep = first_msg.skip_sleep;

        let runtime = transient_execution::setup_runtime(&registration)
            .await
            .map_err(|err| Status::internal(format!("setup runtime: {err}")))?;

        // Channels bridging the VM driver and the gRPC streams.
        let (out_tx, out_rx) = mpsc::channel::<Result<proto::WorkflowStreamResponse, Status>>(32);
        let (settlement_tx, settlement_rx) = mpsc::channel::<
            waymark_vm_driver_core::PromiseSettlement<waymark_system_vm::ReadyValue, ()>,
        >(32);

        let shared = Arc::new(Mutex::new(transient_execution::SharedState {
            skip_sleep,
            ..Default::default()
        }));

        // Spawn the VM driver on a dedicated OS thread.
        let driver_handle = transient_execution::spawn_driver(
            runtime,
            out_tx,
            settlement_rx,
            settlement_tx.clone(),
        ...
        tokio::spawn(async move {
            if let Err(err) =
                transient_execution::settlement_feeder(&mut in_stream, &settlement_tx, shared).await
            {
                tracing::warn!(?err, "settlement feeder exited");
            }
        });

        // Monitor the driver thread for completion / errors.
        tokio::spawn(async move {
            match driver_handle.await {
                Err(err) => tracing::warn!(?err, "vm driver exited"),
