...
//! - WAYMARK_OBSERVABILITY_POSTGRES_READ_STATEMENT_TIMEOUT_MS: Statement timeout on the observability read pool (default: 10000)
//! - WAYMARK_ESSENTIAL_METRICS_SAMPLE_INTERVAL_MS: Essential-metrics sampling interval (default: 10000)
//! - WAYMARK_ESSENTIAL_METRICS_SINK_BUFFERS / WAYMARK_ESSENTIAL_METRICS_SINK_MAX_BATCH / WAYMARK_ESSENTIAL_METRICS_SINK_MAX_DELAY_MS / WAYMARK_ESSENTIAL_METRICS_SINK_FLUSHERS: Essential-metrics write batching (defaults: 3 / 64 / 5000 / 1)
//! - WAYMARK_ESSENTIAL_METRICS_RETENTION_MS / WAYMARK_ESSENTIAL_METRICS_RETENTION_SWEEP_INTERVAL_MS: Essential-metrics retention (defaults: 604800000 / 600000)
//! - WAYMARK_OBSERVABILITY_EVENTS_SINK_BUFFERS / WAYMARK_OBSERVABILITY_EVENTS_SINK_MAX_BATCH / WAYMARK_OBSERVABILITY_EVENTS_SINK_MAX_DELAY_MS / WAYMARK_OBSERVABILITY_EVENTS_SINK_FLUSHERS: Observability-events write batching (defaults: 3 / 256 / 1000 / 1)
//! - WAYMARK_OBSERVABILITY_EVENTS_RETENTION_MS / WAYMARK_OBSERVABILITY_EVENTS_RETENTION_SWEEP_INTERVAL_MS: Observability-events retention (defaults: 604800000 / 600000)
//! - WAYMARK_OBSERVABILITY_EVENTS_RECORD_SNAPSHOT_PERSISTED: Record the snapshot_persisted events (default: false)
...
-> Result<waymark_fn_main_lifecycle::CleanShutdown<()>, waymark_fn_main_common::Error> {
    ...
    // any point of the startup is latched for the listener instead of
    // ending the process on the spot, and has no other effect until the
    // listener runs.
