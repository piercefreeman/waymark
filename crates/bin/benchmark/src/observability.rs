...
/// Wait a bounded time for the pipelines to end — their token is
/// cancelled by the caller — then count the events recorded so far.
pub async fn shutdown(observability: Observability) -> Result<i64, color_eyre::eyre::Report> {
    let Observability {
        handles,
        emitter,
