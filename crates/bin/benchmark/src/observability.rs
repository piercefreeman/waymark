...
/// half never served: the essential-metrics sampler needs the recorder,
/// not the endpoint.
...
{
    let (sampling_handle, _prometheus_handle) =
    ...
    // Provision before the reset: a store the migrations have not reached
    // yet has nothing for the reset to clear, and the count below would
    // then include what an earlier run left behind.
