...
//! Boot Rappel Singleton - Ensures a single bridge server instance is running.
//!
//! This binary:
...
async fn main() -> Result<()> {
    ...
    info!(host = %host, base_port, "probing for existing rappel bridge");

    if let Some(port) = probe_existing_server(&host, base_port).await {
        info!(port, "found existing rappel bridge");
    ...
    info!(port = grpc_port, "rappel bridge started");
...
fn grpc_host_port() -> (String, u16) {
    if let Ok(addr) = env::var("RAPPEL_BRIDGE_GRPC_ADDR") {
    ...
    let host = env::var("RAPPEL_BRIDGE_GRPC_HOST").unwrap_or_else(|_| "127.0.0.1".to_string());
    let base_port = env::var("RAPPEL_BRIDGE_GRPC_PORT")
        .ok()
        .or_else(|| env::var("RAPPEL_BRIDGE_BASE_PORT").ok())
