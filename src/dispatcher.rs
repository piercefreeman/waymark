...
impl DispatcherTask {
    ...
    async fn dispatch_action(
        ...
        // Auto-complete synthetic nodes that don't need Python execution
        // These are control-flow nodes generated by the IR-to-DAG converter
        if action.function_name == "__gather_sync" || action.function_name == "__conditional_join" || action.function_name == "__loop_head" {
            debug!(
                action_id = %action.id,
                instance_id = %action.instance_id,
                function_name = %action.function_name,
                "auto-completing synthetic node"
            );
            // These nodes collect results from dependencies - the scheduler handles the context aggregation
            ...
            if let Err(err) = completion_tx.send(record).await {
                warn!(?err, "completion channel closed, dropping synthetic node completion");
            }
            return Ok(());
        }
