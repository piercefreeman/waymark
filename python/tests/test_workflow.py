...
@action
async def wf_validate_order(order: dict) -> dict:
    raise NotImplementedError


@action
async def wf_process_payment(validated: dict) -> dict:
    raise NotImplementedError


@action
async def wf_send_confirmation(payment: dict) -> str:
    raise NotImplementedError


def test_multi_action_loop_serializes_to_protobuf() -> None:
    """Multi-action loops should serialize to protobuf with body_graph."""
    ...
    class MultiActionLoopWorkflow(Workflow):
        async def run(self, orders: list) -> list:
            results = []
            for order in orders:
                validated = await wf_validate_order(order=order)
                payment = await wf_process_payment(validated=validated)
                confirmation = await wf_send_confirmation(payment=payment)
                results.append(confirmation)
            return results

    dag = MultiActionLoopWorkflow.workflow_dag()

    # Find the multi_action_loop node
    loop_node = next((node for node in dag.nodes if node.action == "multi_action_loop"), None)
    assert loop_node is not None, "Should have a multi_action_loop node"
    assert loop_node.multi_action_loop is not None

    # Get the protobuf representation
    from rappel.workflow import _build_multi_action_loop_ast

    loop_ast = _build_multi_action_loop_ast(loop_node.multi_action_loop)

    # Verify the body_graph is populated
    assert loop_ast.HasField("body_graph"), "Should have body_graph"
    body_graph = loop_ast.body_graph
    assert len(body_graph.nodes) == 3, "Should have 3 phase nodes"
    assert body_graph.result_variable == "confirmation"

    # Verify phase nodes
    phase_0 = body_graph.nodes[0]
    assert phase_0.id == "phase_0"
    assert phase_0.action == "wf_validate_order"
    assert len(phase_0.depends_on) == 0

    phase_1 = body_graph.nodes[1]
    assert phase_1.id == "phase_1"
    assert phase_1.action == "wf_process_payment"
    assert "phase_0" in phase_1.depends_on

    phase_2 = body_graph.nodes[2]
    assert phase_2.id == "phase_2"
    assert phase_2.action == "wf_send_confirmation"
    assert "phase_1" in phase_2.depends_on

    # Verify kwargs are parsed as expressions
    assert len(phase_0.kwargs) == 1
    assert phase_0.kwargs[0].arg == "order"
