"""
IR Builder - Converts Python workflow AST to Waymark IR (ast.proto).
...
valid Waymark IR structures. Control flow bodies (try, for, while, if branches)
...
RECOMMENDATIONS = {
    ...
    "gather_return_exceptions": (
        ...
        "are represented consistently in Waymark's IR.\n\n"
        "Update your call:\n\n"
...
def _discover_action_names(module: Any) -> Dict[str, ActionDefinition]:
    ...
    for attr_name in dir(module):
        ...
        action_name = getattr(attr, "__waymark_action_name__", None)
        action_module = getattr(attr, "__waymark_action_module__", None)
...
def _cmp_op_to_ir(op: ast.cmpop) -> Optional[ir.BinaryOperator]:
    ...
    mapping = {
        ...
        # Identity comparisons are treated as equality in waymark since we deal
