...
LOGGER = configure_logger("waymark.bridge")
...
def _boot_command() -> list[str]:
    override = os.environ.get("WAYMARK_BOOT_COMMAND")
    if override:
...
def _env_grpc_port_override() -> Optional[int]:
    ...
    override = os.environ.get("WAYMARK_BRIDGE_GRPC_PORT")
    if not override:
        return None
    try:
        return int(override)
    except ValueError as exc:  # pragma: no cover
        raise RuntimeError(f"invalid WAYMARK_BRIDGE_GRPC_PORT value: {override}") from exc


def _boot_singleton_blocking() -> int:
    """Boot the singleton and return the gRPC port."""
    command = _boot_command()
    if os.environ.get("WAYMARK_BOOT_COMMAND") is None:
        command[0] = _ensure_boot_binary(command[0])
    with tempfile.NamedTemporaryFile(mode="w+", suffix=".txt") as f:
        output_file = Path(f.name)

        command.extend(["--output-file", str(output_file)])
        LOGGER.info("Booting waymark singleton via: %s", " ".join(command))
        ...
        try:
            ...
            raise RuntimeError("unable to boot waymark server") from exc
...
def _grpc_target() -> str:
    ...
    explicit = os.environ.get("WAYMARK_BRIDGE_GRPC_ADDR")
    if explicit:
        return explicit

    # Otherwise, use host + port
    host = os.environ.get("WAYMARK_BRIDGE_GRPC_HOST", DEFAULT_HOST)
...
async def execute_workflow(payload: bytes) -> bytes:
    ...
    os.environ.setdefault("WAYMARK_BRIDGE_IN_MEMORY", "1")
    async with ensure_singleton():
        stub = await _workflow_stub()
