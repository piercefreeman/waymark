...
<code_feedback>
<rule>Exercise runnable example applications through their real user-facing path and runtime boundary before calling them complete. Good: submit the Next.js form against a live Waymark bridge and assert the VM result. Bad: rely only on compiler tests and a production build.</rule>
...
<rule>Require an explicit action runtime at every parser, compiler, and test-construction boundary; never silently default unspecified actions to Python. Good: `parse_program(source, ActionRuntime::Python)`. Bad: `parse_program(source)` with an implicit runtime.</rule>
<rule>Each worker process owns exactly one action runtime, worker pool, and bridge; select the runtime from shared configuration and keep language-specific bringup in separate functions. Good: `match config.action_worker { Python { .. } => run_python_workers(...), JavaScript { .. } => run_javascript_workers(...) }`. Bad: one process conditionally creating both Python and JavaScript pools.</rule>
<rule>Name tests after the invariant under test, not the example runtime used to exercise it. Good: `dispatch_encodes_the_declared_runtime`. Bad: `javascript_dispatch_works` when the assertion applies to every runtime.</rule>
<rule>Centralize recursive AST inspection and mutation in the AST-owning helper crate instead of duplicating traversal in integration tests or consumers. Good: `required_action_runtimes(&program)`. Bad: local recursive matches over every statement and expression variant.</rule>
<rule>Keep integration-test dependencies in the layer that owns the behavior; do not broaden a bringup crate with database, parser, worker, and runtime dependencies for redundant end-to-end coverage. Good: focused compiler and worker-pool tests in their owning crates. Bad: a bringup test assembling the entire system to re-test runtime metadata.</rule>
</code_feedback>
