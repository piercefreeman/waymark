...
<rule>Declare backend trait methods as `fn … -> impl Future<Output = …> + Send + '_` (`+ Send` where callers need a `Send` future; the lifetime is whatever the future borrows) and implement them with `async fn`, instead of BoxFuture-based signatures. Good: `trait ApplyRetention { fn apply_retention(&self, cutoff: DateTime<Utc>) -> impl Future<Output = Result<u64, Self::Error>> + Send + '_; }` with `async fn apply_retention(&self, cutoff: DateTime<Utc>) -> Result<u64, Self::Error> { … }` in the impl. Bad: `fn apply_retention<'a>(&'a self, cutoff: DateTime<Utc>) -> BoxFuture<'a, Result<u64, Self::Error>>;`</rule>
...
<rule>Record metrics through the `metrics::` macros where the event happens, named `waymark_<crate>_…`; don't build per-pool tracker structs around them. Good: `metrics::counter!("waymark_worker_process_pool_actions_completed_total").increment(1);` Bad: per-pool `WorkerThroughputTracker`/`LatencyTracker` structs.</rule>
