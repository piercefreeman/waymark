...
<rule>Don't run blocking filesystem/IO on the async runtime in a hot path; resolve and cache the materialized values once up front (e.g. an async `prepare()` that runs the probing under `spawn_blocking`) and have the hot path read the cache. Good: `Config::prepare().await` yields a `PreparedConfig` that a pure `prepare_spawn_params` reads. Bad: re-running `is_dir()`/`current_dir()`/`var("PATH")` on every spawn inside the trait impl.</rule>
<rule>Prefer fallible paths in failure-tolerance scopes, resolving them before the values are passed into hot or infallible paths; don't `unwrap`/`expect`/panic inside an infallible trait method or hot path. Good: `std::env::current_dir().map_err(PrepareError::CurrentDir)?` inside an async `prepare()`. Bad: `std::env::current_dir().expect("...")` inside `prepare_spawn_params`.</rule>
</code_feedback>
