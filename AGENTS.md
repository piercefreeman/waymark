...
<rule>Don't run blocking filesystem/IO on the async runtime in a hot path; resolve and cache the materialized values once up front (e.g. an async preparation step that runs the probing under `spawn_blocking`) and have the hot path read the cache. Good: `waymark_worker_python::resolve(config).await` yields a `SpecFactory` that a pure `prepare_spawn_params` reads. Bad: re-running `is_dir()`/`current_dir()`/`var("PATH")` on every spawn inside the trait impl.</rule>
