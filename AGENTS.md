...
<rule>Lower compound control-flow syntax as nested semantic regions instead of interleaving optional behavior throughout one lowering routine. Good: lower `try`/`except`/`finally` as an outer `try`/`finally` region around an inner `try`/`except`. Bad: scatter `finally_block.is_some()` checks through exception-handler construction and flow joins.</rule>
...
<rule>Represent interleaved frame cleanup scopes with one ordered unwind stack and one depth. Good: exception handlers and shared-state returns are unwind entries discarded together when control escapes. Bad: parallel stacks whose separate depths must be synchronized by compiler-generated bytecode.</rule>
</code_feedback>
