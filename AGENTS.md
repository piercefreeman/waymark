...
<rule>Don't `unwrap`/`expect`/panic on an infallible trait method or hot path; push the fallible/blocking initialization into a separate async constructor that returns a typed error. Good: `std::env::current_dir().map_err(PrepareError::CurrentDir)?` inside `prepare()`. Bad: `std::env::current_dir().expect("...")` inside `prepare_spawn_params`.</rule>
</code_feedback>
