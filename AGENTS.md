...
<code_feedback>
<rule>Author UI fixtures as the API's own event or sample streams and derive every displayed fact from them with the production derivation code. Good: `instances = scenarios.map((s) => deriveInstance(s.vmId, eventsFor(s.vmId), now))`. Bad: fixture objects shaped like the screen (`{ name: "OrderFulfillment", status: "failing", duration: "1.42 s" }`) that can show facts the API never reports.</rule>
