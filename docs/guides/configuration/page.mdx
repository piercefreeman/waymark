...
set `WAYMARK_DATABASE_URL`, `WAYMARK_ACTION_RUNTIME`, and the selected
language's action source. Everything else has a default tuned for a
...
| `WAYMARK_WORKER_COUNT` | Number of language worker processes. | Host CPU count |
...
| `WAYMARK_JAVASCRIPT_ACTION_BUNDLE` | Generated ESM action bundle; required when the runtime is `javascript`. | Unset |
...
<Note>
`WAYMARK_PYTHON_USER_MODULE` is the one most people miss on first setup. The
...
| Variable | Description | Default |
| --- | --- | --- |
| `WAYMARK_WORKER_GRPC_ADDR` | gRPC bind for the selected language worker bridge. | `127.0.0.1:24118` |
...
`WAYMARK_MAX_ACTION_LIFECYCLE` controls how many actions a worker
