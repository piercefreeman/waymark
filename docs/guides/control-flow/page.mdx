...
The comprehension element can be an `@action` call or a workflow helper
method - `*[self.process(item) for item in items]` fans out one helper
call per item, and each helper can await actions internally.
